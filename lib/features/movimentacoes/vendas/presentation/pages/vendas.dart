...
import 'add_venda.dart';
import 'detail_venda.dart';
import 'edita_venda.dart';
import 'graficos_venda.dart';
...
class _VendasState extends State<Vendas> with SingleTickerProviderStateMixin {
  ...
  Widget _buildList() {
    if (_listPageController.isLoading && _listPageController.vendas.isEmpty) {
    ...
    if (_listPageController.vendas.isEmpty) {
      return ListView(
        children: const [
          SizedBox(height: 120),
          Center(child: Text('Nenhuma venda cadastrada.')),
        ],
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        itemCount: _listPageController.vendas.length,
        itemBuilder: (context, index) {
          ...
          return _VendaCard(
            ...
            onOpen: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => DetailVenda(venda: venda)),
              );
            },
            onEdit: () {
              Navigator.push(
  ...
  void _showModalBottomSheetExcluir(BuildContext context, VendaEntity venda) {
...
class _VendaCard extends StatelessWidget {
  ...
  String get _destination {
    final parts = [
      ...
      venda.municipio,
      venda.destinos
          .map((item) => item.destinoReference?.nome ?? item.tipo)
