...
class CompraModel extends CompraEntity {
  const CompraModel({
    required super.id,
  ...
  factory CompraModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        .whereType<Map>()
        .map(
          (item) => CompraAnimalModel.fromJson(Map<String, dynamic>.from(item)),
        )
    ...
    return CompraModel(
      ...
      appFazendasId: int.tryParse(json['app_fazendas_id']?.toString() ?? ''),
      ...
      valorFrete: json['valor_frete']?.toString(),
      valorComissao: json['valor_comissao']?.toString(),
      ...
      animais: animais,
      tipoCompra: json['tipo_compra']?.toString(),
      fornecedor: json['fornecedor']?.toString(),
      idFornecedor: int.tryParse(json['id_fornecedor']?.toString() ?? ''),
