...
class TrocaCategoriaModel extends TrocaCategoriaEntity {
  const TrocaCategoriaModel({
    required super.id,
  ...
  factory TrocaCategoriaModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        .whereType<Map>()
        .map(
          (item) => TrocaCategoriaAnimalModel.fromJson(
            Map<String, dynamic>.from(item),
          ),
        )
        .toList(growable: false);

    return TrocaCategoriaModel(
