...
import 'package:costeira/features/movimentacoes/infra/models/morte_animal_model.dart';
import 'package:costeira/features/movimentacoes/infra/models/movimentacao_reference_model.dart';

class MorteModel extends MorteEntity {
  const MorteModel({
    required super.id,
  ...
  factory MorteModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        .whereType<Map>()
        .map(
          (item) => MorteAnimalModel.fromJson(Map<String, dynamic>.from(item)),
        )
