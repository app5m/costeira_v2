...
import 'package:costeira/features/movimentacoes/infra/models/abigeato_animal_model.dart';
import 'package:costeira/features/movimentacoes/infra/models/movimentacao_reference_model.dart';

class AbigeatoModel extends AbigeatoEntity {
  const AbigeatoModel({
    required super.id,
  ...
  factory AbigeatoModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        .whereType<Map>()
        .map(
          (item) =>
              AbigeatoAnimalModel.fromJson(Map<String, dynamic>.from(item)),
        )
        .toList(growable: false);

    return AbigeatoModel(
