...
class AbortoModel extends AbortoEntity {
  const AbortoModel({
    required super.id,
  ...
  factory AbortoModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        .whereType<Map>()
        .map(
          (item) => AbortoAnimalModel.fromJson(Map<String, dynamic>.from(item)),
        )
