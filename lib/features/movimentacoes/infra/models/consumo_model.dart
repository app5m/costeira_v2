...
import 'package:costeira/features/movimentacoes/infra/models/consumo_animal_model.dart';
import 'package:costeira/features/movimentacoes/infra/models/movimentacao_reference_model.dart';

class ConsumoModel extends ConsumoEntity {
  const ConsumoModel({
    required super.id,
  ...
  factory ConsumoModel.fromJson(Map<String, dynamic> json) {
    final animais = (json['animais'] as List<dynamic>? ?? const [])
        ...
        .map(
          (item) =>
              ConsumoAnimalModel.fromJson(Map<String, dynamic>.from(item)),
        )
