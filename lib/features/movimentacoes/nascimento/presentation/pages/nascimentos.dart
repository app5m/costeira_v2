...
import 'add_nascimento.dart';
import 'detail_nascimento.dart';
import 'editar_nascimento.dart';
import 'graficos_nascimento.dart';
...
class _NascimentosState extends State<Nascimentos>
  ...
  Widget _buildList() {
    ...
    if (_listPageController.nascimentos.isEmpty) {
      return ListView(
        children: const [
          SizedBox(height: 120),
          Center(child: Text('Nenhum nascimento cadastrado.')),
        ],
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        itemCount: _listPageController.nascimentos.length,
        itemBuilder: (context, index) {
          final nascimento = _listPageController.nascimentos[index];
...
class _NascimentoCard extends StatelessWidget {
  const _NascimentoCard({
  ...
  String get _subtitle {
    final matriz = nascimento.animais
        .where((item) => item.tipo.toLowerCase().contains('matriz'))
        .firstOrNull;
    final parts = [
      matriz?.brinco,
      matriz?.categoria?.nome,
      nascimento.potreiro?.nome,
      nascimento.lote?.nome,
    ].where((item) => item?.trim().isNotEmpty == true).map((item) => item!);

    if (parts.isEmpty) {
      return 'Matriz nao informada';
    }
    return parts.join(' - ');
  }

  String? _sexoLabel(int sexo) {
    if (sexo == 1) {
      return 'Macho';
    }
    if (sexo == 2) {
