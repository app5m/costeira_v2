...
class _EditAnimalState extends State<EditAnimal> {
  ...
  final EditAnimalController _controller = Modular.get<EditAnimalController>();
  ...
  void initState() {
    super.initState();
    AppLogger.info('ANIMAIS EDIT PAGE: INIT STATE');
    ...
    if (animal != null) {
      AppLogger.info('ANIMAIS EDIT PAGE: CARREGANDO ANIMAL ID=${animal.id}');
      _controller.setInitialAnimal(animal);
      _selectedSexo = animal.sexo;
      _selectedCategoryId = animal.appAnimaisCategoriasId;
      _selectedSubcategoryId = animal.appAnimaisSubcategoriasId;
      _selectedBaseRacialId = animal.utBasesRaciaisId;
      _selectedLotId = animal.appAnimaisLotesId;
      _selectedStatus = _animalStatuses.contains(animal.status)
          ? animal.status
          : null;
      _brincoController.text = animal.brinco ?? '';
      _pesoController.text = animal.peso?.toString() ?? '';
      _obsController.text = animal.obs ?? '';
    }
    _loadInitialData();
  }
  ...
  @override
  void dispose() {
    AppLogger.info('ANIMAIS EDIT PAGE: DISPOSE');
  ...
  Future<void> _loadInitialData() async {
    AppLogger.info(
      'ANIMAIS EDIT PAGE: CARREGANDO LISTAS E LOTES PARA SEXO=$_selectedSexo',
    );
    try {
      await Future.wait([
      ...
      AppLogger.success('ANIMAIS EDIT PAGE: LISTAS E LOTES CARREGADOS');
    } catch (_) {
      AppLogger.error('ANIMAIS EDIT PAGE: ERRO AO CARREGAR LISTAS/LOTES');
    }
  }
  ...
  String get _selectedLotLabel =>
      _selectedLot?.nome ?? widget.animal?.lote?.nome ?? 'Selecionar lote';
  ...
  bool get _hasChanges {
    ...
    return _selectedSexo != animal.sexo ||
        _selectedCategoryId != animal.appAnimaisCategoriasId ||
        _selectedSubcategoryId != animal.appAnimaisSubcategoriasId ||
        _selectedBaseRacialId != animal.utBasesRaciaisId ||
        _selectedLotId != animal.appAnimaisLotesId ||
        _normalizedValue(_brincoController.text) !=
            _normalizedValue(animal.brinco) ||
        _normalizedPesoValue(_pesoController.text) !=
            _normalizedPesoValue(animal.peso?.toString()) ||
        _normalizedValue(_obsController.text) != _normalizedValue(animal.obs) ||
        _selectedStatus != animal.status;
  }

  Future<void> _submit() async {
    AppLogger.info('ANIMAIS EDIT PAGE: VALIDANDO FORMULARIO PARA EDICAO');
    final animal = widget.animal;
    if (animal == null) {
      _showMessage('Animal não encontrado para edição.');
      return;
    }
    ...
    try {
      AppLogger.info(
        'ANIMAIS EDIT PAGE: ENVIANDO EDICAO DO ANIMAL ID=${animal.id}',
      );
      ...
      if (result.isSuccess) {
        AppLogger.success(
          'ANIMAIS EDIT PAGE: ANIMAL ATUALIZADO COM SUCESSO MSG=${result.message}',
        );
      ...
      AppLogger.error('ANIMAIS EDIT PAGE: ERRO AO ATUALIZAR ANIMAL');
      _showMessage(
        _controller.errorMessage ?? 'Não foi possível atualizar o animal.',
      );
    }
  }

  Future<void> _openLotSelection() async {
    AppLogger.info('ANIMAIS EDIT PAGE: ABRINDO SELETOR DE LOTES');
    ...
    if (result.shouldAddLot) {
      AppLogger.info('ANIMAIS EDIT PAGE: USUARIO ESCOLHEU ADICIONAR LOTE');
  ...
  void _showMessage(String message, {bool isError = true}) {
    AppLogger.warning('ANIMAIS EDIT PAGE: EXIBINDO MENSAGEM $message');
  ...
  void _syncSelectedCategory() {
    AppLogger.debug(
      'ANIMAIS EDIT PAGE: SINCRONIZANDO CATEGORIA E SUBCATEGORIA',
    );
            ...
            child: Column(
              ...
              children: [
                ...
                _buildDropdown<ListCategoryEntity>(
                  ...
                  onChanged: (value) {
                    AppLogger.info(
                      'ANIMAIS EDIT PAGE: CATEGORIA ALTERADA PARA ID=${value ?? 'NULL'}',
                    );
                    setState(() {
                ...
                if (_requiresSubcategory)
                  _buildDropdown<ListSubcategoryEntity>(
                    ...
                    onChanged: (value) {
                      AppLogger.info(
                        'ANIMAIS EDIT PAGE: SUBCATEGORIA ALTERADA PARA ID=${value ?? 'NULL'}',
                      );
                      setState(() {
                ...
                _buildDropdown<ListItemEntity>(
                  ...
                  onChanged: (value) {
                    AppLogger.info(
                      'ANIMAIS EDIT PAGE: BASE RACIAL ALTERADA PARA ID=${value ?? 'NULL'}',
                    );
                    setState(() {
                ...
                _buildStatusDropdown(
                  ...
                  onChanged: (value) {
                    AppLogger.info(
                      'ANIMAIS EDIT PAGE: STATUS ALTERADO PARA ${value ?? 'NULL'}',
                    );
                    setState(() {
            ...
            Expanded(
              child: RadioListTile<int>(
                ...
                onChanged: (value) {
                  ...
                  AppLogger.info('ANIMAIS EDIT PAGE: SEXO ALTERADO PARA MACHO');
                  setState(() {
                    _selectedSexo = value;
                    _selectedCategoryId = null;
                    _selectedSubcategoryId = null;
            ...
            Expanded(
              child: RadioListTile<int>(
                ...
                onChanged: (value) {
                  ...
                  AppLogger.info('ANIMAIS EDIT PAGE: SEXO ALTERADO PARA FEMEA');
                  setState(() {
                    _selectedSexo = value;
                    _selectedCategoryId = null;
                    _selectedSubcategoryId = null;
